...
But it may also be of use as a method to start computing answers to a user
...
course $p$ may of course also be empty, which corresponds to the case that the
user has not yet typed any prefix.
...
$\Argmax*[k]$ is a auxiliary used to model the $k$ words that maximize the
...
is performed.
But the ability to calculate word prediction in a manner faster than the
...
\begin{enumerate}
  ...
  \item We will apply well known \emph{top-$k$ joining techniques}
    ...
    prediction by avoiding the enumeration of all words in the vocabulary.
\end{enumerate}
...
allows: the system will always predict exactly those words that maximize the
probability.
...
\emph{Top-$k$ join queries} are those top-$k$ queries that produce
