...
Technically whenever a $\overline{\SumArg}_i$ value is updated, the upper bound
...
every time a $\overline{\SumArg}$ value is updated.
This is not shown in the code in the algorithm.
...
This mean one only needs to recalculate the upper bound in places where the
...
So to obtain a $\SumArg^h_i(w)$ the $i$th completion trie has to be queried with
$\DerivedHistory[i]$.
...
another and than starting over.
...
$\bar{\SumArg}_i$ needs to be zeroed once the $i$th trie is depleted.
