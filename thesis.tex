...
\documentclass[m,bachelor,binding,palatino]{WeSTthesis}
...
% For text colorization (included by tikz)
%\usepackage{color}
%\usepackage[usenames,dvipsnames]{xcolor}

% Pseudocode
\usepackage[noend]{algpseudocode} % Omits end statements in algorithms
\usepackage[chapter]{algorithm}   % Have algorithm numbering per chapter
\renewcommand{\algorithmicrequire}{\textbf{Input:}}
\renewcommand{\algorithmicensure}{\textbf{Output:}}
% Make comments gray
\definecolor{comment}{rgb}{0.4, 0.4, 0.4}
\algrenewcomment[1]{\hfill\textcolor{comment}{\(\triangleright\) #1}}
% http://tex.stackexchange.com/questions/74880/algorithmicx-package-comments-on-a-single-line
\algnewcommand{\LineComment}[1]{\State \textcolor{comment}{\(\triangleright\) #1}}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndFor
    \LineComment{\todo{This can probably be simplified considerably (better check with source)}}
  \end{algorithmic}
\end{algorithm}
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Ensure $\SumWeight_i^h$
      ...
      \For{$i \gets 2 \mathinner{\ldotp \ldotp} N$}
        %\State $\gamma^\prime \gets \gamma(\History)$
        %\State $\History \gets \History\text{.backoff()}$
      ...
      \EndFor
      \State \todo{Replace fraction with $x^{-1}$ for better visual?}
  \end{algorithmic}
\end{algorithm}
